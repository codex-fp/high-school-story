...
promotion_note: >-
  Promoted from the gds-ux workflow artifact after explicit request to move the
  accepted UX contract into durable project documentation. This document owns
  information architecture, HUD behavior, phone behavior, dialogue behavior,
  travel UX, lesson UX, and accessibility-floor rules for High School Story.
---

# High School Story UX Experience Document
...
repeating upstream product and narrative scope. When gameplay, architecture, or
narrative rules change how the player should read or use the interface, this
document is the durable UX owner.
...
Current platform contract: desktop JVM, keyboard and mouse first. The UI should
preserve the possibility of future controller support by keeping focus order and
menu structure coherent, but controller-first glyph systems are not part of the
...
| Teacher-attention hint | Lessons | Semi-readable only. Scene acting plus light UI support, no full meter. |
...
- subtle phone slide-in and slide-out
