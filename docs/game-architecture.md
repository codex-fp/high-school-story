...
promotion_note: >-
  ...
  framework-oriented technical architecture document.
---
...
This is the durable game-architecture source of truth for High School Story. It
is intentionally game-architecture focused: it describes how the game should be
structured as a playable social simulation, how gameplay systems relate to each
...
loop, lessons, relationships, phone read models, locations, authored content,
save/recovery, content scaling, and game-level risks.
...
|---|---|---|---|
| GA-1 | Use a discrete-time simulation model. | The GDD says state advances on actions, turns, travel, sleep, and calendar transitions rather than second-by-second management. | Accepted |
| GA-2 | Treat `GameState` as the canonical coordination model. | Beats, actions, phone apps, saves, endings, and previews need one shared state surface. | Accepted |
| GA-3 | Keep beat gating inside beat definitions or action definitions. | Scene eligibility should not drift into launchers, UI, or unrelated orchestration code. | Accepted |
| GA-4 | Represent the daily loop as phases with allowed action contexts. | Morning, school, afternoon, evening, and sleep have different valid choices and constraints. | Accepted |
| GA-5 | Model lessons as a shared turn loop parameterized by subject identity. | The GDD rejects bespoke subject minigames and requires subject feel through weighting and tone. | Accepted |
| GA-6 | Model relationships as descriptive stages plus arc progress. | Player-facing stages avoid raw-number optimization while still allowing deterministic gates. | Accepted |
| GA-7 | Make the phone a read model over game state and authored content. | Calendar, map, profiles, messages, classes, and memories should expose fair planning information diegetically. | Accepted |
| GA-8 | Use compact location records with practical and emotional functions. | Place-first routine and memory need stable location identity without large open-world scope. | Accepted |
| GA-9 | Persist checkpoints at meaningful calendar boundaries. | Academic failure recovery requires saves from about 3 days, 2 weeks, and 2 months earlier. | Accepted |
| GA-10 | Keep authored content modular and state-addressable. | Three years, 10 classmates, clubs, phone content, and memory album require scalable content units. | Accepted |
...
These are implementation recommendations. Treat them as accepted architecture
guidance, while leaving exact class names, formulas, and persistence formats to
...
1. Should `GameState` become the explicit canonical aggregate, or should the
   game use separate domain states composed by a higher-level run state?
2. What is the first accepted save-file format and versioning approach?
3. Which state categories must exist before HSS-4 through HSS-8 implementation
   continues?
4. How much subject identity should be data-driven in the first lesson
   implementation?
5. What exact classmate profile categories are public, discovered, private, or
   route-specific?
6. Should the phone read models be implemented as in-memory projections first,
   with persistence only at the underlying state layer?
7. Which content registry format should be used when authored beats outgrow
   direct wiring?
8. Which preview scenarios are mandatory for validating the first architecture
   slice?
